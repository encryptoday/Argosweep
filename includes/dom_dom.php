...
function file_get_html(
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 30); // Timeout (seconds)

    // Handle context options if provided (you can adjust these depending on your needs)
    if ($context) {
    ...
    // Check for errors
    if (curl_errno($ch)) {
        // Handle cURL errors here if needed
        $dom->clear();
        curl_close($ch);
        return false;
    }
    ...
    if ($http_code != 200 || empty($contents) || strlen($contents) > $maxLen) {
        $dom->clear();
        curl_close($ch);
        return false;
    }
